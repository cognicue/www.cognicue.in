...
affdex.getCogniDotJsLocation = function() {
  return window.location.origin+"/assets/demo/facs/js/";
};
...
affdex.Detector.prototype.start = function() {
  if (!this.isRunning) {
    var url = affdex.getCogniDotJsLocation();
    XHRWorker(url+"cogni-worker.js", function(worker) {
      this.worker = worker;
...
affdex.CameraDetector = function(element, imgW, imgH, faceMode) {
  ...
  self.start = function() {
    if(!self.isRunning) {
      ctor();
      var url = affdex.getCogniDotJsLocation() + adapterJSVersion;
