    ...
    const c = (n, t) => {
        ...
        _ = (n, m) => {
            ...
            let l = u[e[m].mr_name].max,
                ...
                s = u[e[m].mr_name].min;
            null === l ? u[e[m].mr_name].max = a : l < a && (u[e[m].mr_name].max = a), null === s ? u[e[m].mr_name].min = a : s > a && (u[e[m].mr_name].min = a), u[e[m].mr_name].mean = null === t ? a : (t * r + a) / (r + 1)
        },
        d = (e, n) => {
            let m = 0;
            for (let a = 0; a < e.length; a++) m += Math.pow(n - e[a], 2);
            return e.length >= 2 ? (m /= e.length - 1, Math.sqrt(m)) : null
        };
    return {
        log: n => {
            ...
            }), s = !0, c(a, o) ? Promise.resolve(a) : Promise.reject("Data did not pass validation."))).then(h(t)).then(e => {
                console.log(e)
            })["catch"](e => {
                console.log("Unable to perform aggregation:"), console.log(e)
            })
        },
        show: () => s ? {
            summary: u,
